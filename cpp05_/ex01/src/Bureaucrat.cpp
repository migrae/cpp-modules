...
/*   Updated: 2023/07/06 09:01:32 by mgraefen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::increment(int inc) {
	if(_grade - inc < GRADE_MAX)
		...
		_grade -= inc;
		std::cout << _name << " incremented by " << std::setw(3) << std::left << inc << " : Bureaucrat now has Grade: " << _grade << std::endl;
	}
}
...
void Bureaucrat::decrement(int dec) {
		...
		_grade += dec;
		std::cout << _name << " decremented by " << std::setw(3) << dec << " : Bureaucrat now has Grade: " << _grade << std::endl;
	}
}

void Bureaucrat::signForm(Form &sign_form) const {
  try {
    sign_form.beSigned(*this);
    std::cout << _name << " signs " << sign_form.getName() << std::endl;
  } catch (std::exception &e) {
    std::cout << _name << " can't sign " << sign_form.getName()
