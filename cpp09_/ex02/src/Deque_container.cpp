...
std::deque<std::string> Deque_container::_str_loader;
std::deque<std::pair<int, int> > Deque_container::_pairs;
std::deque<int> Deque_container::_final_numbers;
std::deque<unsigned int> Deque_container::_jt_idx;
int Deque_container::struggler = -1;
double Deque_container::elapsed_time = 0;

// public

void Deque_container::sort(int argc, char **argv) {
...
void Deque_container::print_elapsed_time() {
  std::cout << "Time to process a range of " << _str_loader.size()
            << " elements with std::deque : " << elapsed_time * 1e6 << "us"
            << std::endl;
}

// private

void Deque_container::is_sorted() {
...
bool Deque_container::is_num(std::string str) {
...
void Deque_container::load_deque(void) {
  int first;
  int second;
...
void Deque_container::swap_pairs(std::pair<int, int> &p) {
  if (p.second > p.first) std::swap(p.first, p.second);
}

void Deque_container::print_pairs(void) {
  for (DPairIterator it = _pairs.begin(); it != _pairs.end(); ++it)
    std::cout << "FIRST: " << it->first << " SECOND: " << it->second
              << std::endl;
}

void Deque_container::print_final_numbers(void) {
  std::cout << "BEFORE: ";
...
void Deque_container::divide_pairs(Pair_deque &p) {
...
void Deque_container::merge_pairs(Pair_deque &left, Pair_deque &right,
                                  Pair_deque &p) {
...
void Deque_container::push_sorted_a_chain() {
  for (size_t i = 0; i < _pairs.size(); i++) {
    if (_pairs[i].second != -1) _final_numbers.push_back(_pairs[i].first);
  }
}

void Deque_container::create_jt_idx() {
...
unsigned int Deque_container::binary_search(unsigned int start,
